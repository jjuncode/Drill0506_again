...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            points.append((event.x, TUK_HEIGHT - 1 - event.y))  # 클릭된 위치를 추가
...
def set_new_target_arrow():
    ...
    if points:  # points 리스트안에 남아있는 점이 있으면,
        ...
        action = 3 if action == 1 else 2  # 이전에 소년이 우측으로 이동중이었으면, IDLE 동작이 우측바라봄
        frame = 0
        target_exists = False
...
def update_world():
    ...
    if target_exists:
        if t <= 1.0:
            ...
            cx, cy = hx, hy  # 캐릭터의 위치를 목적지 위치와 정확히 일치시킴.
            del points[0]  # 목표 지점에 왔기 때문에, 더 이상 필요없는 점을 삭제
            set_new_target_arrow()
    elif points:  # points에 점이 있을경우 새로운 지점을 계속해서 설정
        set_new_target_arrow()
...
while running:
    ...
    update_world()  # 월드 안의 객체들의 상호작용, update
